...
#%% Evaluation 
def test(model, dataset_conf, results_path, n_imfs , num_arti_FRMs, allRuns = True):
    ...
    for sub in range(n_sub): # (num_sub): for all subjects, (i-1,i): for the ith subject.
        # Load data
        # print(dataset_conf.get('LOSO'))
        _, _, _, X_test, _, y_test_onehot = get_data(data_path, sub, dataset, LOSO, isStandard)      
        
        # -----augumenting.....
        X_test,y_test_onehot = emd_tta(X_test, y_test_onehot, dataset_conf, n_imfs, num_arti_FRMs)
...
def run():
    ...
    test(model, dataset_conf, results_path, n_imfs = 1 , num_arti_FRMs = 1)
