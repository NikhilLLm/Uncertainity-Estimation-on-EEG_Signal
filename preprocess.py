...
def get_data(path, subject, dataset = 'EEG_CSV', classes_labels = ['all'], LOSO = False, isStandard = True, isShuffle = True):
    ...
    if LOSO:
        ...
        #     X_test, y_test = load_HGD_data(path, subject+1, False)
            ...
            X_train, X_test, y_train, y_test = load_data_csv(subject, path,train_size=0.6,n_samples=1125,classes_labels = classes_labels)
        else:
            raise Exception("'{}' dataset is not supported yet!".format(dataset))

    # train_len = int(0.1 * len(X_train))
    # test_len = int(0.1 * len(X_test))
